...
interface UseIndexedDBResult {
  ...
  getAllValues: (tableName: string) => Promise<any[]>;
  putValue: (tableName: string, value: object) => Promise<IDBValidKey | null>;
  putBulkValues: (tableName: string, values: object[]) => Promise<any[]>;
  updateValue: (params: {
    tableName: string;
    id: number;
    newItem: any;
...
export const useIndexedDB = (
  ...
  const getAllValues = (tableName: string): Promise<any[]> => {
    return new Promise((resolve, reject) => {
      try {
        const store = getTransaction(tableName, "readonly");
